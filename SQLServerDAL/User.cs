...
namespace FWSync.SQLServerDAL
{
    public class User : IUser
    {
        ...
        //静态常量
        private const string SQL_COUNT_USER_BY_USERNAME = "SELECT COUNT(*) FROM [ArduinoTestDB].[dbo].[User] WHERE UserName = @UserName";
        private const string SQL_COUNT_USER_BY_USERNAME_AND_PASSWORD = "SELECT COUNT(*) FROM [ArduinoTestDB].[dbo].[User] WHERE UserName = @UserName AND PassWord = @PassWord";
        private const string SQL_INSERT_USER = "INSERT INTO [ArduinoTestDB].[dbo].[User] VALUES (@UserName,@PassWord);select SCOPE_IDENTITY();";
